...
public class SalesServiceFacadeImpl implements SalesServiceFacade {
	...
	@Override
	public HashMap<String, String> getPredictNextDay(){
		ArrayList<ContractInfoTO> predictNextDay = contractDAO.getPredictNextDay();
		Map<String, List<Map<String, Object>>> result = new HashMap<>();

		for (ContractInfoTO item : predictNextDay) {
			String code = item.getCustomerCode();
			String date = item.getContractDate();
			String qtyStr = item.getContractRequester();
			int quantity;

			try {
				quantity = Integer.parseInt(qtyStr.trim());
			} catch (Exception e) {
				System.out.println("⚠️ 수량 변환 오류: " + qtyStr);
				continue;
			}
			System.out.println("\n" + quantity + "\n");

			Map<String, Object> oneRow = new HashMap<>();
			oneRow.put("date", date);
			oneRow.put("quantity", quantity);

			result.computeIfAbsent(code, k -> new ArrayList<>()).add(oneRow);

			System.out.println("\n" + oneRow + "\n");
			System.out.println("\n" + result + "\n");
		}

		// 2. Python 서버로 POST 전송
		RestTemplate rest = new RestTemplate();
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<Map<String, List<Map<String, Object>>>> entity = new HttpEntity<>(result, headers);

		System.out.println("\n" + entity + "\n");

		String pythonUrl = "http://localhost:8001/predict";
		ResponseEntity<Map> response = rest.postForEntity(pythonUrl, entity, Map.class);

		// 3. Python으로부터 받은 예측 결과 가공
		Map<String, Object> body = response.getBody();
		HashMap<String, String> finalResult = new HashMap<>();

		if (body != null && body.containsKey("prediction")) {
			Map<String, Double> predictions = (Map<String, Double>) body.get("prediction");
			for (String code : predictions.keySet()) {
				finalResult.put(code, String.valueOf(predictions.get(code)));
			}
		}
