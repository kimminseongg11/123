...
const actualWorkOrderClick = async () => {
  ...
  if (!propz.DialogData || Object.values(propz.DialogData).some((value) => value === '')) {
    ...
    try {
      const { mrpNo, mrpGatheringNo, workPlaceCode, productionProcessCode } = propz.DialogData;
      await productionStore().FETCH_ACTUAL_WORKORDER_BUTTON(mrpNo, mrpGatheringNo, workPlaceCode, productionProcessCode );
