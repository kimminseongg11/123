...
const getMrpGatherList = async () => {
  ...
  // console.log("res.data.gridRowJson : " + res.data.gridRowJson);
  console.log("res.data.gridRowJson:", JSON.stringify(res.data.gridRowJson, null, 2));
...
watch([item], () => {
  ...
  for (let i = 0; i < item.value.length; i++) {
    console.log("item.value : " + item.value[i].mrpNo);
    list.push(item.value[i].mrpNo);
    mpsNoList.value = list;
  }

  console.log("mpsNoList : ", toRaw(mpsNoList));
  ...
  <div>
    <div class="header_wrap">
      <p style="font-weight: bold">
        품목구분이 <span class="span">반제품,완제품, 재공품</span>인 경우 필요수량은
