...
export const productionStore = defineStore("productionStore", {
  ...
  actions: {
    ...
    // 실제 작업지시
    async FETCH_ACTUAL_WORKORDER_BUTTON(mrpNo: string, mrpGatheringNo: string, workPlaceCode: string, productionProcessCode: string ) {
      try {
        const response = await getActualWorkOrderButton(mrpNo, mrpGatheringNo, workPlaceCode, productionProcessCode );
